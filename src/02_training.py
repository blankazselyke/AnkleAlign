...
# Importok
from config import CSV_PATH, OUTPUT_DIR, MODEL_SAVE_PATH, EPOCHS, BATCH_SIZE, LEARNING_RATE, WEIGHT_DECAY, EARLY_STOPPING_PATIENCE, LR_SCHEDULER_FACTOR, LR_SCHEDULER_PATIENCE, SEED, VAL_SPLIT, TEST_SPLIT, CSV_PATH_TRAIN, CSV_PATH_VAL, CSV_PATH_TEST
...
def main():
    ...
    # --- JAVÍTÁS 1: Külön transzformációk Tanításhoz és Validációhoz ---
    """
    train_transform = transforms.Compose([
        transforms.Resize((224, 224)),
        transforms.RandomRotation(3),
        transforms.RandomAffine(degrees=0, translate=(0.02, 0.02), scale=(0.95, 1.0)),
    ...
    class AugmentedAnkleDataset(Dataset):
        """
        Each original image is expanded into N augmented samples.
        index // N -> original image index
        index % N  -> which augmentation to apply
        """
    ...
    # --- JAVÍTÁS 4: A megfelelő transform átadása ---
    #train_ds = AnkleDataset(train_df, OUTPUT_DIR, transform=train_transform)
    ...
    logger.info(f"Effective training samples: {len(train_ds)}")
    ...
    logger.info(str(model))  # Logs full architecture
